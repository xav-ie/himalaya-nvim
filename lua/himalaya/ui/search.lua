...
local WHEN_PRESETS = {
  {
    label = 'today',
    resolve = function()
      return 'date ' .. today_str(0)
    end,
  },
  {
    label = 'yesterday',
    resolve = function()
      return 'date ' .. today_str(-1)
    end,
  },
  {
    label = 'past 3 days',
    resolve = function()
      return 'after ' .. today_str(-3)
    end,
  },
  {
    label = 'past week',
    resolve = function()
      return 'after ' .. today_str(-7)
    end,
  },
  {
    label = 'past 2 weeks',
    resolve = function()
      return 'after ' .. today_str(-14)
    end,
  },
  {
    label = 'past month',
    resolve = function()
      return 'after ' .. today_str(-30)
    end,
  },
  {
    label = 'past 3 months',
    resolve = function()
      return 'after ' .. today_str(-90)
    end,
  },
  {
    label = 'this week',
    resolve = function()
      return 'after ' .. week_start()
    end,
  },
  {
    label = 'this month',
    resolve = function()
      return 'after ' .. os.date('%Y-%m') .. '-01'
    end,
  },
  {
    label = 'this year',
    resolve = function()
      return 'after ' .. os.date('%Y') .. '-01-01'
    end,
  },
}
...
for i, f in ipairs(FIELDS) do
  if f.complete == 'flag' then
    FLAG_LINE = i - 1
  end
  if f.complete == 'when' then
    WHEN_LINE = i - 1
  end
  if f.complete == 'folder' then
    FOLDER_LINE = i - 1
  end
end
...
function M.open(callback, prev_query, current_folder, account)
  ...
  local function recompose_query()
    if not query_subscribed then
      return
    end
    local values = {}
    for i = 1, #FIELDS do
      values[i] = get_line(i - 1)
    end
    ...
    on_lines = function(_, _, _, first_line, lastline, new_lastline)
      ...
      elseif first_line == QUERY_LINE then
        ...
        vim.schedule(function()
          ...
          if edit_gen[QUERY_LINE] ~= gen then
            return
          end
  ...
  vim.keymap.set({ 'n', 'i' }, '<C-x>', function()
    ...
    if not field or (not field.keyword and not field.complete) then
      return
    end
  ...
  if prev_query and prev_query ~= '' and last_state then
    ...
    propagating = false
    for k, v in pairs(last_state.negated) do
      negated[k] = v
    end
