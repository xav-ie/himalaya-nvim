describe('himalaya.domain.email.tree', function()
  ...
  describe('build (reverse)', function()
    ...
    it('mirrors branch connectors: ┌─ for first, └─ for last', function()
      -- Normal:   Root, ├─A, └─B
      -- Reversed: ┌─B, └─A, Root
      local edges = {
        { {id='0'}, {id='1', from='A', subject='Root', date='2024-01-01 10:00:00+00:00'}, 0 },
      ...
      local rows = tree.build(edges, { reverse = true })
      tree.build_prefix(rows, { reverse = true })
      -- C is first branch child at this level → ┌─
    ...
    it('mirrors 3-way branch: ┌─ first, ├─ middle, └─ last', function()
      local edges = {
        { {id='0'}, {id='1', from='A', subject='Root', date='2024-01-01 10:00:00+00:00'}, 0 },
      ...
      tree.build_prefix(rows, { reverse = true })
      -- Reversed: D, C, B, Root
      assert.are.equal('\xe2\x94\x8c\xe2\x94\x80', rows[1].prefix)  -- ┌─ first
