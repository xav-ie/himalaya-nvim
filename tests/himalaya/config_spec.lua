describe('himalaya.config', function()
  ...
  it('returns defaults when setup not called', function()
    ...
    assert.are.equal('!', c.flags.flagged)
    assert.are.equal('*', c.flags.unseen)
    assert.are.equal('R', c.flags.answered)
    assert.are.equal('@', c.flags.attachment)
    assert.are.equal('FLGS', c.flags.header)
  end)
